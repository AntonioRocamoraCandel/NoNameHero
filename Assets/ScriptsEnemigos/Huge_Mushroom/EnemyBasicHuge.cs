...
public class EnemyBasicHuge : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        movimiento =
            Vector2.right *
            velocidadMovimiento; // Define el movimiento a la derecha como el vector de velocidad por defecto
    ...
    void FixedUpdate()
    {
        ...
        if (target != null || !isCollision)
        {
            ...
            if (distance <= 5)
            {
                ...
                transform.position =
                    Vector2.MoveTowards(transform.position, newPosition, Time.deltaTime * velocidadMovimiento);
            }
            else
            {
                isDetected = false;
